...
const repeatMyParam = () => {
  ...
  axios.get("http://localhost:3000/repeat/I-Love-Coding").then((res) => {
    ...
    let repeatP = document.getElementById("repeat-text");
    repeatP.textContent = res.data;
    repeatP.style.display = "block";
  });
};
document
...
const queryEnd = () => {
  axios
    .get("http://localhost:3000/query-test?myquery=OMG-it-WORKED")
    .then(({ data }) => console.log(data));
};
document.getElementById("query-button").addEventListener("click", queryEnd);
////////////////
//INTERMEDIATE//
////////////////
